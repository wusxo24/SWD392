...
const ChildHealth = ({ isOpen, onClose, trackingData, setTrackingData }) => {
  ...
  const allFields = [
    "Height", "Weight", "BMI", "BMIZScore", "BMIResult",
    "HeadCircumference", "WaistCircumference", "HipCircumference",
    "BicepsCircumference", "TricepsCircumference", "ChestCircumference",
    "ThighCircumference", "CalfCircumference"
  ];
                    ...
                    {allFields.map((key) => (
                      <li key={key} className="flex items-center space-x-2">
                        ...
                        {bmiFields.includes(key) ? (
                          <span className="font-semibold text-gray-800">{formatValue(details[key])}</span>
                        ) : editingDates[date] ? (
                          <input
                            type="number"
