...
const NewsDetail = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 text-sm mt-2">By {news.author || "Admin"} - {new Date(news.date).toLocaleDateString()}</p>
      
      <img src={news.banner} alt={news.title} className="w-full mt-6 rounded-lg shadow-lg" />
      ...
      {/* Related News */}
      <h2 className="text-2xl font-semibold text-gray-800 mt-12">Related News</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        {relatedNews.map((item) => (
          <Link key={item._id} to={`/news/${item._id}`} className="block bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-300">
            <img src={item.banner} alt={item.title} className="w-full h-40 object-cover rounded-lg" />
